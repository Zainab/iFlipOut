...
import com.out.iflipout.tasks.FetchChannel;
import com.out.iflipout.utils.Logger;
import com.out.iflipout.utils.UrlBuilder;

public class ChannelList extends ListActivity implements OnClickListener,
OnItemClickListener, OnChannelDownload, ListView.OnScrollListener  {
	private String url = null;
	private UrlBuilder builder = new UrlBuilder();
	List<Channel> documents = null;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		url = builder.getChannelURL();
		Logger.i(url);
		new FetchChannel(this, this).execute(url);
		documents = new ArrayList<Channel>(); 
		Logger.i("Documents Size " + documents.size());
	}

	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		Channel document = documents.get(position);
		Intent intent = new Intent(Intent.ACTION_VIEW);
		intent.setAction("android.intent.action.VIEW"); 
        intent.addCategory("android.intent.category.BROWSABLE");
		intent.setData(Uri.parse(document.getLink().trim()));
	...
	public void onDownloadComplete(ListAdapter adapter, List<Channel> documents) {
		...
		this.setListAdapter(new ChannelAdapter(this, this.documents));
